(() => {
   "use strict";
   var __webpack_modules__ = ([, ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
   ...
   (() => {
      ...
      var _modules_functions_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1); _modules_functions_js__WEBPACK_IMPORTED_MODULE_0__.isWebp(); document.addEventListener("DOMContentLoaded", function () {
         ...
         if (previewElements.length === 0 || !customCursor) return;

         function enableCustomCursor() {
            previewElements.forEach(preview => {
               preview.addEventListener('mouseenter', showCursor); preview.addEventListener('mouseleave', hideCursor); preview.addEventListener('mousemove', moveCursor)
            })
         }
         ...
         function disableCustomCursor() {
            ...
            previewElements.forEach(preview => {
               preview.removeEventListener('mouseenter', showCursor); preview.removeEventListener('mouseleave', hideCursor); preview.removeEventListener('mousemove', moveCursor)
            })
         }
      ...
      function initMobileSwipers() {
         const sliderElements = document.querySelectorAll('.review__gallery-slider');
      ...
      window.addEventListener('load', initMobileSwipers); window.addEventListener('resize', initMobileSwipers);
      ...
      document.addEventListener('DOMContentLoaded', () => {
         ...
         if (previews.length === 0 && submenuLinks.length === 0 && reviews.length === 0 && backButtons.length === 0) {
            return
         }

         if (!history.state) {
            history.replaceState({ slide: 0 }, '', '#slide-0');
         }
         ...
         function showReviewById(id) {
            if (reviews.length > 0) {
               ...
               if (activeReview) {
                  ...
                  const video = activeReview.querySelector('video');
                  if (video) {
                     video.currentTime = 0;
            ...
            // ❗ Не добавляем в историю, если это вызов из popstate
            if (!isPopstate) {
               history.pushState({ review: id, slide: 2 }, '', `#review-${id}`);
            }
         }
         ...
         if (previews.length > 0) {
            previews.forEach(preview => {
               preview.addEventListener('click', () => {
                  const target = preview.dataset.preview; showReviewById(target)
               })
            })
         }
         if (submenuLinks.length > 0) {
            ...
            backButtons.forEach(button => {
               button.addEventListener('click', () => {
                  ...
                  if (review && review.classList.contains('show')) {
                     ...
                     history.pushState({ slide: mainSlider.activeIndex }, '', `#slide-${mainSlider.activeIndex}`);
                  }
               });
            });
         ...
         function updateActiveSlideLink(index) {
            const slideLinks = document.querySelectorAll('[data-slide]');
            slideLinks.forEach(link => {
               link.classList.toggle('active', +link.dataset.slide === index);
            });
         }
         ...
         window.addEventListener('popstate', (event) => {
            ...
            const isReviewHash = hash.startsWith('#review-');
            const isSlideHash = hash.startsWith('#slide-');

            if (!state?.review && document.querySelector('.review.show')) {
            ...
            if (state?.review) {
               showReviewById(state.review);

               if (submenu) submenu.classList.add('opened');
            }
            else if (typeof state?.slide === 'number') {
               mainSlider.slideTo(state.slide);
               updateActiveSlideLink(state.slide);

               if (submenu && state.slide !== 2) {
                  submenu.classList.remove('opened');
               }
            }
            else if (isSlideHash) {
               const index = +hash.replace('#slide-', '');
               if (!isNaN(index)) {
                  mainSlider.slideTo(index);
                  updateActiveSlideLink(index);

                  if (submenu) {
                     submenu.classList.toggle('opened', index === 2);
                  }
               }
            }

            if (!state && !hash) {
               mainSlider.slideTo(2);
               updateActiveSlideLink(2);
               history.replaceState({ slide: 2 }, '', '#slide-2');
               if (submenu) submenu.classList.add('opened');
            }
      ...
      document.addEventListener('DOMContentLoaded', () => {
         ...
         slideLinks.forEach(link => {
            link.addEventListener('click', async (e) => {
               ...
               if (reviewId) {
                  const review = document.querySelector(`.review[data-review="${reviewId}"]`);
                  if (review) {
      ...
      function goToSlide(index) {
         ...
         mainSlider.slideTo(index);
         history.pushState({ slide: index }, '', `#slide-${index}`);
      }
      ...
      function animateReviewIn(review) {
         ...
         gsap.fromTo(review.querySelectorAll('.review__image img, .review__image video'), {
      ...
      function wrapWords(element) {
         ...
         element.innerHTML = '';
         words.forEach(word => {
            const span = document.createElement('span'); span.textContent = word; span.style.display = 'inline-block'; span.style.marginRight = '0.3em'; element.appendChild(span)
         });
      ...
      document.addEventListener('DOMContentLoaded', () => {
         const videoContainers = document.querySelectorAll('.video-review'); videoContainers.forEach(container => {
            const poster = container.querySelector('.video-review__poster');
            const playButton = container.querySelector('.video-review__play');
            ...
            const playVideo = () => {
               ...
               poster.style.display = 'none'; playButton.style.display = 'none'; video.style.display = 'block'; video.play()
            }; poster.addEventListener('click', playVideo); playButton.addEventListener('click', playVideo)
         })
      })
   })()
})();